		...
		if (this.hasBonus) {
				missiles.push(
					...
					new Missile({
						position: {
							// Position X du missile au centre du joueur
							x: (this.position.x+10) + this.width / 2,
							// Position Y du missile alignée avec le joueur
							y: this.position.y,
						},
					}),
	...
	update({ velocity, player }) {
		if (this.image) {
			...
			if (
				...
				// Message affiché si l'alien atteint le bas de l'écran
				gameOver();
			}
		}
...
const shootalienbonuss = () => {
	console.log(bonuss);
	// Gérer les pouvoir des aliens
	bonuss.forEach((bonus, index) => {
		// Si le missile est sorti de l'écran (en bas)
		if (bonus.position.y + bonus.height >= world.height) {
			// Supprimer le missile après un délai de 0ms (immédiatement)
			setTimeout(() => {
				bonuss.splice(index, 1);
			}, 0);
		} else {
			// Sinon, mettre à jour la position du missile
			bonus.update();
		...
		// Gérer les collisions entre les missiles des aliens et le joueur
		if (
			bonus.position.y + bonus.height >= player.position.y &&
			// Vérifier la collision entre le missile alien et le joueur (Check collision between alien missile and player)
			bonus.position.y <= player.position.y + player.height &&
			bonus.position.x >= player.position.x &&
			bonus.position.x + bonus.width <= player.position.x + player.width
		) {
			// Collision détectée
			// Supprimer le missile alien
			bonuss.splice(index, 1);
...
if (!isPaused) {
		...
		grids.forEach((grid, indexGrid) => {
			...
			if (frames % 50 === 0 && grid.invaders.length > 0) {
				...
				if (alienMissileShot === 10) {
					console.log("BONUS SHOT")
					// Un alien tire un pouvoir
					grid.invaders[
						Math.floor(Math.random() * grid.invaders.length)
					].shootBonuss(bonuss);
